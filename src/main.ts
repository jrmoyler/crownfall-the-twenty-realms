...
import { animateRig, createRiggedActor, disposeActor, setRigState } from './render/characterRig';
...
class RealmGame {
  ...
  constructor(private readonly canvas: HTMLCanvasElement, private readonly civ: Civilization, private readonly mode: Mode, private readonly onEnd: (result: RunResult) => void) {
    ...
    this.world(); this.player = createRiggedActor(civ, true); this.player.position.set(0, 0, 5.5); this.scene.add(this.player);
  ...
  private world(): void { buildEnvironment({ scene: this.scene, decor: this.decor, civ: this.civ }); }
  ...
  private summon(count: number): void { for (let i = 0; i < count; i += 1) { const ally = createRiggedActor(this.civ, false); const a = (i / count) * Math.PI * 2; ally.position.copy(this.player.position).add(new THREE.Vector3(Math.cos(a), 0, Math.sin(a))); ally.scale.setScalar(.72); this.scene.add(ally); this.allies.push({ mesh: ally, ttl: 15 + Math.random() * 8, hit: 0 }); } }
  ...
  private update(dt: number): void { this.animation += dt; this.attackCooldown -= dt; this.dodgeCooldown -= dt; this.invulnerable -= dt; this.cooldowns.forEach((_, i) => { this.cooldowns[i] = Math.max(0, this.cooldowns[i] - dt); }); this.stamina = Math.min(100, this.stamina + dt * 13); this.command = Math.min(100, this.command + dt * (this.shrine > .65 ? 3 : .7));
    ...
    this.camera.position.lerp(new THREE.Vector3(this.player.position.x * .33, 14, 15 + this.player.position.z * .33), .05); this.camera.lookAt(this.player.position.x * .3, 0, this.player.position.z * .3);
    ...
    this.enemies.forEach((enemy) => animateRig(enemy.mesh, this.animation, enemy.speed));
    this.allies.forEach((ally) => animateRig(ally.mesh, this.animation, 1));
    animateEnvironment(this.decor, this.animation, Math.min(1, this.enemies.length / 10));
